...
def build_adj_diag(nodes, graphs, idx, dataset_name):
    ...
    for i in range(graphs):
        lower_idx_new[i] = lower_idx[i]+i #riscalo l'indice perchè ho inserito 600 nuove righe
        upper_idx_new[i] = upper_idx[i]+i #riscalo l'indice perchè ho inserito 600 nuove righe
...
def load_batch_graph_data(batch_drug,num_nodes, num_graphs, dim_feats,smile_graph,drug_smile,drug_dict):

    b_edge,b_idx,b_f = get_batch_graph(batch_drug,smile_graph,drug_smile,drug_dict)
    

    adj_matrix = build_adj_diag1(b_edge,num_nodes,num_graphs,b_idx)

    node_feats = build_feats_vertConc1(b_idx,b_f,num_graphs,dim_feats)

    #graph_labels = build_labels_vertConc(num_graphs, num_classes, num_nodes, global_nodes_idx, dataset_name) #codifica one-hot
    
    return adj_matrix, node_feats, b_idx

def build_adj_diag1(edge,nodes, graphs, idx):
    #vanno preparati 600 nodi globali, uno per grafo
    nodes_tot = nodes+graphs
    fake_matrix = np.array([[0 for i in range(nodes)] for k in range(graphs)])
    #preparazione righe aggiuntive per i nodi globali
    ...
    for i in range(len(ind1)):
        ...
        v = ind2[i]
        u = int(u)      #vanno letti come stringhe
        v = int(v) #aggiustamenti per eliminare la virgola del file 
        adj_matrix[u-1][v-1] = 1 
    #inserimento righe aggiuntive
    #print("inserting global node rows:")
    for i in range(graphs):
...
def build_feats_vertConc1(idx, matrix, graphs, dim_feats):
    ...
    feats_matrix = sp.csr_matrix(feats_matrix)
    return feats_matrix
def build_feats_basic1(matrix): 

    feats_matrix = matrix
    feats_matrix = sp.csr_matrix(feats_matrix)
    return feats_matrix

def get_batch_graph(batch_name,smile_graph,drug_smile,drug_dict):
    batch_edge=[]
    batch_idx =[]
    batch_features = []
    batch_idx.append(0)
    for  i,name in enumerate(batch_name):
        edge = np.array(smile_graph[drug_smile[drug_dict[name]]][2])
        feature = np.array(smile_graph[drug_smile[drug_dict[name]]][1])
        batch_features = batch_features+[list(i) for i in list(feature)]
        e_size = edge.shape[0]+batch_idx[-1]
        edge=edge+i*96
        batch_edge=batch_edge+[list(i) for i in list(edge)]
        batch_idx.append(e_size)
    return batch_edge,batch_idx,batch_features